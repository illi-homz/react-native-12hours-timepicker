...
const AnalogClock12: FC<AnalogClock12Props> = ({
	...
	const getAngleWrapper = (e: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
		...
		container.current?.measure((...args) => {
			...
			hoursForNumberColor !== currentHour && setCurrentHour(hoursForNumberColor);
		});
	};

	const onSetClockEnd = (e: GestureUpdateEvent<PanGestureHandlerEventPayload>) => {
		container.current?.measure((...args) => {
			const angle = getAngleFromHandler(args, e, topPadding);
			const [hours, minutes, magnetAngle] = getHoursesAndMinutesFromAngle(angle);
		...
		<GestureDetector gesture={gesture}>
			<View style={[styles.container, style]} ref={container}>
				<Svg
					...
					{hoursesList.map((idx) => {
						return (
							<HourDot idx={idx} key={idx} fill={colors.gray} stroke={colors.white1} containerWidth={containerWidth} />
						);
					})}
					...
					{hoursesList.map((idx) => {
						return (
							<HourDot idx={idx} key={idx} fill="transparent" stroke={colors.white1} containerWidth={containerWidth} />
						);
					})}
				</Svg>
				...
				<View style={styles.numbersContainer} pointerEvents="none">
					{hoursesList.map((idx) => {
						...
						if (currentHour === idx + 1 || (currentHour === 0 && idx === 11)) {
